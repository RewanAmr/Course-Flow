...
if ($stmt->rowCount()) {
    ...
    $getNameSql = "SELECT student_name FROM studentsignup WHERE email = ?";
    $getNameStmt = $connect->prepare($getNameSql);
    $getNameStmt->bindParam(1, $email);
    $getNameStmt->execute();

    if ($getNameStmt->rowCount()) {
        $studentData = $getNameStmt->fetch(PDO::FETCH_ASSOC);
        ...
        $mail->Body = <<<END
        Hello $studentName,
        Click <a href="http://localhost/CourseFlow/Student/reset-password.php?token=$token">here</a> 
...
?>
                        ...
                        <!-- Back to StudentLogin.php button -->
                        <a href="StudentLogin.php" class="btn btn-secondary mt-4" style="height:30px;width:150px; background-color: transparent; color: white;">Back to Home</a>
